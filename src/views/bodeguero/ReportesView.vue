  ...
  <div class="space-y-6">
    <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 shadow-xl">
      ...
      <div class="flex flex-wrap gap-4">
        ...
        <button
          @click="generarExcelInventario"
          :disabled="cargandoExcelInv"
        ...
        <button
          @click="generarExcelDanadas"
          :disabled="cargandoExcelDanadas"
          class="bg-orange-600 hover:bg-orange-500 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2"
        >
    ...
    <div class="min-h-screen text-white">
      <div class="bg-gray-900 border border-white/10 p-6 rounded-2xl shadow-xl max-w-md">
        ...
        <apexchart
          type="donut"
          height="300"
          :options="estadoPrestamosOptions"
          :series="estadoPrestamosSeries"
        />
      </div>
    </div>
  </div>
...
const generarExcelUsuarios = async () => {
  try {
    ...
    console.error(error)('Hubo un error al intentar generar el Excel de usuarios.')
  } finally {
...
const generarPDFInventario = async () => {
  try {
    ...
    const inventarioConImagenes = await Promise.all(
      inventarioRaw.map(async (item) => {
